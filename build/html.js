...
function postProc(n, idx, ctx) {
  ...
  ctx.tag.add(n.tagName)
  n.classList.forEach(v => ctx.class.add(v))
  /*
	for (let a in attrs) {
		ctx.attr.add('['+a+']');
...
function parse(html) {
  ...
  build(tokens, (n, idx) => postProc(n, idx, ctx))

  return ctx
}

module.exports.getSibsOfType = getSibsOfType
